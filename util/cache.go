...
const CacheDirectoryName = ".gdr"

var CacheDirectory string = filepath.Join(GuessUnixHomeDir(), CacheDirectoryName)

var CacheFile string = filepath.Join(CacheDirectory, "cache.json")

var ClientFile string = filepath.Join(CacheDirectory, "client.json")
...
func loadCache() (map[string][]byte, error) {
	if _, err := os.Stat(CacheFile); os.IsNotExist(err) {
		...
		f, err := os.OpenFile(CacheFile, os.O_RDONLY|os.O_CREATE, 0600)
		if err != nil {
			log.Fatal(err)
			return nil, err
		}
